const MarketingAjax = (() => {
  ...
  function updateDivElements(nizNekretnina, tip) {
    ...
    if (
      nizNekretnina &&
      nizNekretnina.nizNekretnina &&
      Array.isArray(nizNekretnina.nizNekretnina)
    ) {
  ...
  function impl_osvjeziPretrage(divNekretnine) {
    ...
    if (globalniNizNekretninaPretrage.includes(-1)) {
      ...
      nizNekretnina = nizNekretnina.filter(
        (element) => !globalniNizNekretninaPretrage.includes(element)
      );
    }
  ...
  function impl_osvjeziKlikove(divNekretnine) {
    ...
    if (globalniNizNekretninaKlikovi.includes(-1)) {
      ...
      // Ako globalniNizNekretninaPretrage ne sadrži -1, izbaci elemente iz nizNekretnina koji se nalaze u globalniNizNekretninaPretrage
      nizNekretnina = nizNekretnina.filter(
        (element) => !globalniNizNekretninaKlikovi.includes(element)
      );
    }
  ...
  function impl_klikNekretnina(idNekretnine) {
    ...
    if (nekretninaElement) {
      ...
      // Dodajemo forward slash na početak rute
      ajaxRequest(
        "POST",
        `/marketing/nekretnina/${idNekretnine}`,
        null,  // null umjesto praznog objekta {}
        (error, data) => {
          if (error) {
      ...
      // Opciono: Redirektaj na stranicu s detaljima
